...
object ProducerForTwitterClient {

  def main(args: Array[String]): Unit = {
    new ProducerForTwitterClient().run()
  }
}

class ProducerForTwitterClient {

  // Creating a logger
  var logger: Logger = LoggerFactory.getLogger(classOf[ProducerForTwitterClient].getName)
  ...
  def run(): Unit = {
    ...
    while (!client.isDone) {
      ...
      if (msg != null) {
        val output = ExtractUsefulData.extractTweetFromJson(msg)
        if(output!="-1") {
          ...
          producer.send(new ProducerRecord("Kafka_topic_tweets", null, output), new Callback() {
